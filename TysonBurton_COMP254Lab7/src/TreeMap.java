...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
    ...
    //====================================================================
    //Original Method With Recursion
    /*
    ...
    */
    ...
    private Position<Entry<K,V>> treeSearch(Position<Entry<K,V>> p, K key)
    {
        while (true)
        {
            //Key is not found return the final leaf
            if (isExternal(p)) return p;

            //Compare the searched key to he element stored in the current node
            int comp = compare(key, p.getElement());

            //Key is found return its current position
            if (comp == 0) return p;

            //Search the left subtree
            else if (comp < 0) p = left(p);
